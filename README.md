# Image Cropper

複数の画像を同じ構図で一括切り抜きし、ZIPファイルとしてまとめてダウンロードできるWebツールです。
1枚目の画像で行った切り抜き設定をすべての画像に自動適用し、面倒な反復作業を数秒で完了させます。

## 🚀 主な機能

- **スマート一括トリミング**: 最初の画像で設定した位置・サイズ・変形（Transform）を全画像に自動適用。
- **直感的な操作**: [Cropper.js v2](https://github.com/fengyuanchen/cropperjs) を採用し、スムーズなプレビューと操作感を実現。
- **高速書き出し**: ブラウザ上で並列処理を行い、ZIP形式で一括保存。
- **プライバシー保護**: すべての処理をクライアントサイド（ブラウザ内）で完結。画像サーバーへのアップロードは一切行いません。

## 🛠 技術スタック

- **Framework**: Vue 3 (Composition API)
- **State Management**: Pinia
- **Core Library**: Cropper.js v2 (Web Components)
- **Utilities**: JSZip, FileSaver.js
- **Build Tool**: Vite

## 📅 今後のロードマップ (Planned Features)

ユーザーの利便性向上のため、以下の機能を順次追加予定です。

- [x] **プロパティバーの設置**: 数値入力による精密な座標・サイズ指定機能。
- [x] **個別調整モーダル**: 全体設定の適用後、特定の画像だけをポップアップ画面で微調整できる機能。
- [x] **保存形式・画質の選択**: 出力フォーマット（PNG / JPEG）の切り替えおよび、背景色のカスタマイズ。(背景色の黒で実装中。今後カスタマイズ機能を実装予定)
- [x] **進捗インジケーター**: 大量画像処理時のステータス可視化（「5/10 枚目処理中...」）。
- [x] **切り抜き範囲の制限機能**: 切り抜き枠を画像の内側のみに制限できるオプションを追加します。これにより、画像の縁ギリギリに合わせたリサイズ操作がより簡単になります。
- [x] **JPEG書き出し時の背景色設定**: 透過画像などをJPEGで保存する際、背景色（現在は白固定）を自由に指定できる機能を追加予定です。
- [x] **設定の保存（永続化）**: localStorage を活用し、ブラウザをリロードしても前回の切り抜き設定（アスペクト比など）が保持されるようにします。
- [x] **設定プリセットの保存機能**: 切り抜き設定をプリセットとしてlocalStrageに保存・読み込みができるようにしました。

## 📦 インストールと実行

```bash
# リポジトリをクローン
git clone [https://github.com/takumi-onisi/image-cropper.git](https://github.com/takumi-onisi/image-cropper.git)

# ディレクトリに移動
cd image-cropper

# 依存関係のインストール
npm install

# 開発サーバーの起動
npm run dev
```
